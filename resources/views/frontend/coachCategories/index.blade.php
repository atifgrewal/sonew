        ...
        <div class="col-md-12">
            @can('coach_category_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.coach-categories.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $coachCategory->id }}">
                                        ...
                                        <td>
                                            @can('coach_category_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.coach-categories.show', $coachCategory->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('coach_category_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.coach-categories.edit', $coachCategory->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('coach_category_delete')
                                                <form action="{{ route('frontend.coach-categories.destroy', $coachCategory->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('coach_category_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.coach-categories.massDestroy') }}",
